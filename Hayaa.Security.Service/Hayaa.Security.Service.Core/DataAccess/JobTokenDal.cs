...
///代码效率工具生成，此文件不要修改
/// </summary>
namespace Hayaa.Security.Service.Dao{internal partial class JobTokenDal:CommonDal{
...
internal static int Add(JobToken info){
 string sql = "insert into JobToken(JobId,Token,EndTime) values(@JobId,@Token,@EndTime)";
return Insert<JobToken>(con,sql, info);
}
internal static int Update(JobToken info){
 string sql = "update JobToken set JobId=@JobId,Token=@Token,EndTime=@EndTime where JobTokenId=@JobTokenId";
